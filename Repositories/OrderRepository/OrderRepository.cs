...
namespace PersonalShopper.Repositories.OrderRepository
{
    public class OrderRepository : GenericRepository<Order>, IOrderRepository
    {
        ...
        public async Task<Order> GetOrderById(int id) =>
            await _context.Orders.Where(order => order.OrderId.Equals(id)).FirstOrDefaultAsync();
       
        public async Task<List<Order>> GetAllUserOrders(int id) =>
            await _context.Orders.Where(order => order.UserId.Equals(id)).ToListAsync();
       
        public async Task<List<Order>> GetOrdersByStatus(string status) =>
            await _context.Orders.Where(order => order.OrderStatus.Equals(status)).ToListAsync();
